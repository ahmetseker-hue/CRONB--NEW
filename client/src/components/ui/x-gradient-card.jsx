...
function XCard({
    ...
    content = ["CRONBi ile işletmenizi dijitalleştirin."],
    isVerified = true,
    timestamp = "2025",
    reply = null,
}) {
            ...
            >
                <div
                    className={cn(
                        ...
                        "shadow-sm",
                        "will-change-transform translate-z-0",
                        "before:absolute before:inset-0 before:bg-gradient-to-br before:from-black/[0.02] before:to-black/[0.01] dark:before:from-white/[0.03] dark:before:to-white/[0.01] before:opacity-0 before:transition-opacity before:pointer-events-none before:rounded-xl",
                        "hover:before:opacity-100"
                    )}
                >
                    <div className="flex gap-3">
                        <div className="flex-shrink-0">
                            <div className="h-10 w-10 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-800">
                                <img
                                    src={authorImage}
                                    alt={authorName}
                    ...
                    <div className="mt-3">
                        {content.map((item, index) => (
                            <p
                                key={index}
                                className="text-black dark:text-white/90 text-sm leading-relaxed"
                            >
                                {item}
                            </p>
                        ))}
                        <span className="text-black/50 dark:text-white/50 text-xs mt-2 block">
                            {timestamp}
                        </span>
                    </div>
                    ...
                    {reply && (
                        <div className="mt-4 pt-4 border-t border-black/[0.08] dark:border-white/[0.08]">
                            <div className="flex gap-3">
                                <div className="flex-shrink-0">
                                    <div className="h-8 w-8 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-800">
                                        <img
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-1 flex-wrap">
                                        <span className="font-semibold text-black dark:text-white/90 hover:underline cursor-pointer text-sm">
                                            {reply.authorName}
                                        </span>
                                        {reply.isVerified && (
